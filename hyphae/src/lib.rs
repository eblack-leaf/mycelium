...
fn wrap<'a>(s: &'a str, prefix: &str) -> Option<&'a str> {
    if !s.to_lowercase().starts_with(prefix) { return None; }
...
impl SchemaGraph {
    ...
    pub fn inject(&self, semantics: Semantics) -> SemanticGraph {
        todo!()
    }
}
