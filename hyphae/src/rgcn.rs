...
/// Single R-GCN layer — one Linear per edge type.
...
/// Multi-layer R-GCN stack.
#[derive(Module, Debug)]
pub struct Rgcn<B: Backend> {
    pub layers: Vec<RgcnLayer<B>>,
}

impl<B: Backend> Rgcn<B> {
    pub fn new(feat_dim: usize, hidden_dim: usize, num_layers: usize, device: &B::Device) -> Self {
        let layers = (0..num_layers)
            .map(|i| {
                ...
                RgcnLayer::new(in_dim, hidden_dim, device)
            })
            .collect();
        Self { layers }
    }
